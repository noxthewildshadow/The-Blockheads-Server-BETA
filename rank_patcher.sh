...
is_valid_player_name() {
    local name="$1"
    if [[ -z "$name" ]] || [[ "$name" =~ ^[[:space:]]+$ ]]; then return 1; fi
    if echo "$name" | grep -q -P "[\\x00-\\x1F\\x7F]"; then return 1; fi
    if [[ "$name" =~ ^[[:space:]]+ ]] || [[ "$name" =~ [[:space:]]+$ ]]; then return 1; fi
    if [[ "$name" =~ [[:space:]] ]]; then return 1; fi
    if [[ "$name" =~ [\\\/\|\<\>\:\"\?\*] ]]; then return 1; fi
    local trimmed_name=$(echo "$name" | xargs)
    if [ -z "$trimmed_name" ] || [ ${#trimmed_name} -lt 3 ] || [ ${#trimmed_name} -gt 16 ]; then return 1; fi
    if ! [[ "$trimmed_name" =~ ^[a-zA-Z0-9_]+$ ]]; then return 1; fi
    return 0
}
...
add_to_cloud_admin() {
    ...
    if ! grep -q "^$player_name$" "$cloud_file" 2>/dev/null; then echo "$player_name" >> "$cloud_file"; fi
}
...
apply_rank_to_connected_player() {
    ...
    if [ -n "${rank_already_applied[$player_name]}" ] && [ "${rank_already_applied[$player_name]}" = "$rank" ]; then return; fi
    case "$rank" in
        "MOD")
...
start_disconnect_timer() {
    ...
    if [ "${player_verification_status[$player_name]}" != "verified" ]; then
        ( sleep 1; remove_player_rank "$player_name"; unset disconnect_timers["$player_name"]; ) &
        disconnect_timers["$player_name"]=$!
    else
        ( sleep 10; remove_player_rank "$player_name"; unset disconnect_timers["$player_name"]; ) &
        disconnect_timers["$player_name"]=$!
    fi
}
...
start_password_kick_timer() {
    ...
    (
        ...
        if [ -n "${connected_players[$player_name]}" ]; then
            ...
            if [ -n "$player_info" ]; then
                local password=$(echo "$player_info" | cut -d'|' -f2)
                if [ "$password" = "NONE" ]; then execute_server_command "/kick $player_name"; fi
            fi
        fi
    ) &
...
start_invalid_name_timers() {
    ...
    (
        ...
        while true; do
            ...
            ( sleep 30; execute_server_command "/unban $player_ip"; log_debug "INVALID NAME LOOP: Unbanned $player_ip."; ) &
        done
    ) &
...
cancel_player_timers() {
    ...
    for timer_type in "${timer_types[@]}"; do
        ...
        if [ -n "${active_timers[$timer_key]}" ]; then
            ...
            if kill -0 "$pid" 2>/dev/null; then kill "$pid" 2>/dev/null; log_debug "Killed timer $timer_key (PID: $pid) for $player_name"; fi
...
sync_lists_from_players_log() {
    ...
    if [ -f "$PLAYERS_LOG" ]; then
        while IFS='|' read -r name first_ip password rank whitelisted blacklisted; do
            ...
            if [ "${player_verification_status[$name]}" = "invalid_name" ]; then continue; fi
            if [ "${player_verification_status[$name]}" != "verified" ]; then
                if [ "$rank" != "NONE" ]; then pending_ranks["$name"]="$rank"; fi
                continue
            fi
...
force_reload_all_lists() {
    ...
    while IFS='|' read -r name first_ip password rank whitelisted blacklisted; do
        ...
        if [ -z "${connected_players[$name]}" ]; then continue; fi
        if [ "${player_verification_status[$name]}" = "invalid_name" ]; then continue; fi
        if [ "${player_verification_status[$name]}" != "verified" ]; then continue; fi
        if [ "$rank" != "NONE" ]; then apply_rank_to_connected_player "$name"; fi
    done < "$PLAYERS_LOG"
}
...
handle_invalid_player_name() {
    ...
    if [ -n "$player_ip" ] && [ "$player_ip" != "unknown" ]; then
        if [ -z "$player_name" ]; then
            ...
            log_debug "INVALID NAME: '$player_name'. Allowing connection, starting timers."
            print_warning "Invalid name '$player_name' (IP: $player_ip) connected. Starting monitoring timers."
...
monitor_players_log() {
    ...
    if [ -f "$PLAYERS_LOG" ]; then
        while IFS='|' read -r name first_ip password rank whitelisted blacklisted; do
            name=$(echo "$name" | xargs); rank=$(echo "$rank" | xargs); current_player_ranks["$name"]="$rank"
        done < "$PLAYERS_LOG"
    fi
    while true; do
        if [ -f "$PLAYERS_LOG" ]; then
    ...
    while true; do
        if [ -f "$admin_list" ]; then
            ...
            if [ "$current_admin_checksum" != "$last_admin_checksum" ]; then
                ...
                sleep 2; sync_lists_from_players_log; last_admin_checksum="$current_admin_checksum"
            fi
        fi
        if [ -f "$mod_list" ]; then
            ...
            if [ "$current_mod_checksum" != "$last_mod_checksum" ]; then
                ...
                sleep 2; sync_lists_from_players_log; last_mod_checksum="$current_mod_checksum"
            fi
        fi
        sleep 5
    done
...
monitor_console_log() {
    ...
    local wait_time=0
    while [ ! -f "$CONSOLE_LOG" ] && [ $wait_time -lt 30 ]; do sleep 1; ((wait_time++)); done
    ...
    tail -n 0 -F "$CONSOLE_LOG" | while read -r line; do
        ...
        if [[ "$line" =~ Player\ Connected\ (.*)\ \|\ ([0-9a-fA-F.:]+)\ \|\ ([0-9a-f]+) ]]; then
            ...
            if ! is_valid_player_name "$player_name"; then
                if handle_invalid_player_name "$player_name" "$player_ip" "$player_hash"; then is_invalid_name=1; log_debug "handle_invalid_player_name returned 0, marking as invalid_name=1"; else log_debug "handle_invalid_player_name returned 1, skipping connection logic."; continue; fi
            fi
        ...
        if [[ "$line" =~ ([a-zA-Z0-9_]+):\ (.+)$ ]]; then
            ...
            player_name=$(echo "$player_name" | xargs | tr '[:lower:]' '[:upper:]')
            if [ "${player_verification_status[$player_name]}" = "invalid_name" ]; then continue; fi
            if is_valid_player_name "$player_name"; then
                case "$message" in
                    "!psw "*)
                        if [[ "$message" =~ !psw\ ([^[:space:]]+)\ ([^[:space:]]+)$ ]]; then
                            local password="${BASH_REMATCH[1]}"; local confirm_password="${BASH_REMATCH[2]}"
                            log_debug "$player_name trying to set password."; handle_password_creation "$player_name" "$password" "$confirm_password"
                        else
                            execute_server_command "/clear"; execute_server_command "ERROR: $player_name, invalid format! Example: !psw Mypassword123 Mypassword1abc"
                        fi ;;
                    "!change_psw "*)
                        if [[ "$message" =~ !change_psw\ ([^[:space:]]+)\ ([^[:space:]]+)$ ]]; then
                            local old_password="${BASH_REMATCH[1]}"; local new_password="${BASH_REMATCH[2]}"
                            log_debug "$player_name trying to change password."; handle_password_change "$player_name" "$old_password" "$new_password"
                        else
                            execute_server_command "/clear"; execute_server_command "ERROR: $player_name, invalid format! Use: !change_preplace_psw OLD_PASSWORD NEW_PASSWORD"
                        fi ;;
                    "!ip_change "*)
                        if [[ "$message" =~ !ip_change\ ([^[:space:]]+)$ ]]; then
                            local password="${BASH_REMATCH[1]}"; log_debug "$player_name trying to verify IP."; handle_ip_change "$player_name" "$password" "$current_ip"
                        else
                            execute_server_command "/clear"; execute_server_command "ERROR: $player_name, invalid format! Use: !ip_change YOUR_PASSWORD"
                        fi ;;
                esac
            fi
        fi
        if [[ "$line" =~ cleared\ (.+)\ list ]]; then
            ...
            sleep 2; force_reload_all_lists
        fi
    done
}
...
setup_paths() {
    local port="$1"
    if [ -f "world_id_$port.txt" ]; then WORLD_ID=$(cat "world_id_$port.txt"); else
    ...
    if [ -z "$WORLD_ID" ]; then print_error "World ID file is empty. Cannot continue."; return 1; fi
