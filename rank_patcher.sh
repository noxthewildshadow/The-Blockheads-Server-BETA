...
# Variables globales
declare -g PLAYERS_LOG=""
declare -g CONSOLE_LOG=""
declare -g SCREEN_SESSION=""
declare -g WORLD_ID=""
declare -g PORT=""
declare -g PATCH_DEBUG_LOG=""

# Estados del sistema
declare -g DEBUG_LOG_ENABLED=1
...
declare -A pending_list_creations
...
is_valid_player_name() {
    ...
    [[ -z "$name" ]] && return 1
    [[ "$name" =~ ^[[:space:]]+$ ]] && return 1
    echo "$name" | grep -q -P "[\\x00-\\x1F\\x7F]" && return 1
    ...
    [[ -z "$trimmed_name" ]] && return 1
    [[ ${#trimmed_name} -lt 3 || ${#trimmed_name} -gt 16 ]] && return 1
    [[ ! "$trimmed_name" =~ ^[a-zA-Z0-9_]+$ ]] && return 1
...
# =============================================================================
# CONFIGURACIÓN DE RUTAS Y INICIALIZACIÓN
# =============================================================================
...
execute_server_command() {
    ...
    if send_server_command "$SCREEN_SESSION" "$command"; then
        ...
        log_debug "FAILED to execute command: $command"
        return 1
    fi
}
...
update_player_info() {
    ...
    if [ -f "$PLAYERS_LOG" ]; then
        # Actualizar entrada existente o agregar nueva
...
# =============================================================================
# GESTIÓN DE LISTAS (adminlist, modlist, cloudwide)
# =============================================================================
        ...
        if [ "${player_verification_status[$player]}" = "verified" ]; then
            ...
            if [ -n "$player_info" ]; then
                ...
                if [ "$player_rank" = "$rank" ]; then
                    ...
                    log_debug "Found verified player $player with rank $rank - list creation needed"
                    break
                fi
            fi
        fi
    ...
    case "$rank" in
        "MOD")
            ...
            if [ ! -f "$mod_list" ]; then
                ...
                pending_list_creations["mod"]=1
                (
                    sleep 2
                    execute_server_command "/unmod CREATE_LIST"
                    unset pending_list_creations["mod"]
        ...
        "ADMIN"|"SUPER")
            ...
            if [ ! -f "$admin_list" ]; then
                ...
                pending_list_creations["admin"]=1
                if [ "$rank" = "SUPER" ]; then
                    add_to_cloud_admin "CREATE_LIST"
                    pending_list_creations["cloud"]=1
                fi
                (
                    sleep 2
                    ...
                    if [ "$rank" = "SUPER" ]; then
                        ...
                        unset pending_list_creations["cloud"]
                    fi
                    unset pending_list_creations["admin"]
...
start_rank_application_timer() {
    ...
    if [ -n "${connected_players[$player_name]}" ] && [ "${player_verification_status[$player_name]}" = "verified" ]; then
        ...
        if [ -n "$player_info" ]; then
            ...
            if [ "$rank" != "NONE" ]; then
                log_debug "Player $player_name is verified with rank $rank, proceeding with list creation and rank application"
                
                # Paso 1: Crear lista si es necesario
                create_list_if_needed "$rank"
                
                # Paso 2: Aplicar rango después de 5 segundos
                (
                    sleep 5
                    if [ -n "${connected_players[$player_name]}" ] && [ "${player_verification_status[$player_name]}" = "verified" ]; then
...
start_disconnect_timer() {
    ...
    # Si el jugador no está verificado, limpiar inmediatamente
    if [ "${player_verification_status[$player_name]}" != "verified" ]; then
        log_debug "Player $player_name not verified - cleaning up immediately"
        cleanup_empty_lists_after_disconnect "$player_name"
        return
    fi
    
    # Para jugadores verificados: proceso normal de 15 segundos
    (
        sleep 10
...
handle_ip_change() {
    ...
    if [ -n "$player_info" ]; then
        ...
        # Cancelar temporizadores de IP
        cancel_player_timers "$player_name"
        
        # Aplicar rangos pendientes y sincronizar
...
sync_lists_from_players_log() {
    ...
    # Inicialización en primera sincronización
    ...
    if [ -f "$PLAYERS_LOG" ]; then
        while IFS='|' read -r name first_ip password rank whitelisted blacklisted; do
            ...
            # Aplicar cambios de rango si es necesario
            ...
            # Manejar cambios en blacklist/whitelist
...
apply_rank_changes() {
    ...
    # Aplicar nuevo rango si existe y el jugador está verificado
...
force_reload_all_lists() {
    ...
    while IFS='|' read -r name first_ip password rank whitelisted blacklisted; do
        ...
        # Solo recargar para jugadores verificados
        if [ "${player_verification_status[$name]}" != "verified" ]; then
...
# =============================================================================
# MANEJO DE WHITELIST/BLACKLIST
# =============================================================================

handle_whitelist_change() {
    local player_name="$1" whitelisted="$2" current_ip="$3"
    
    log_debug "Handling whitelist change: $player_name -> $whitelisted (IP: $current_ip)"
...
handle_blacklist_change() {
    ...
    log_debug "Handling blacklist change: $player_name -> $blacklisted"
    ...
    if [ -n "$player_info" ]; then
        ...
        if [ "$blacklisted" = "YES" ]; then
            # Remover rangos si está blacklisteado
            case "$rank" in
                "MOD")
                    execute_server_command "/unmod $player_name"
                    ;;
...
cancel_player_timers() {
    ...
    # Lista de tipos de temporizadores a cancelar
...
monitor_console_log() {
    ...
    tail -n 0 -F "$CONSOLE_LOG" | while read -r line; do
        ...
        if [[ "$line" =~ ([a-zA-Z0-9_]+):\ (.+)$ ]]; then
            ...
            if is_valid_player_name "$player_name"; then
                handle_chat_command "$player_name" "$message" "$current_ip"
            fi
        fi
        
        # Detectar limpieza de listas
...
handle_player_connect() {
    ...
    if [ -z "$player_info" ]; then
        ...
        if [ "$first_ip" = "UNKNOWN" ]; then
            ...
            # Remover rango actual hasta verificación
            if [ "$rank" != "NONE" ]; then
        ...
        # Iniciar aplicación de rango solo si está verificado
        if [ "${player_verification_status[$player_name]}" = "verified" ]; then
...
handle_chat_command() {
    local player_name="$1" message="$2" current_ip="$3"
...
# =============================================================================
# FUNCIONES DE MONITOREO ADICIONALES
# =============================================================================
    ...
    while IFS='|' read -r name first_ip password rank whitelisted blacklisted; do
        ...
        if [ -n "$previous_line" ]; then
            ...
            if [ "$prev_whitelisted" != "$whitelisted" ]; then
                log_debug "Whitelist change detected: $name from $prev_whitelisted to $whitelisted"
                local current_ip="${player_ip_map[$name]}"
                handle_whitelist_change "$name" "$whitelisted" "$current_ip"
            fi
        fi
...
# =============================================================================
# FUNCIONES PRINCIPALES Y CLEANUP
# =============================================================================
