...
declare -A invalid_name_timers # [NUEVO] Para rastrear bucles de kick
...
# [MODIFICADO] Devuelve códigos de estado:
# 0: Válido
# 1: Vacío/Espacios (Ban inmediato)
# 2: Inválido simple (Corto, símbolos, etc. - Ban temporal)
is_valid_player_name() {
    local name="$1"
    
    # Caso 1: String vacío o solo espacios (Ban Inmediato)
    if [[ -z "$name" ]] || [[ "$name" =~ ^[[:space:]]+$ ]]; then
        return 1 # Código 1 para Vacío/Espacios
    fi
    
    # --- Todos los demás casos retornan 2 (Inválido simple) ---
    
...
    
    # Chequeo de longitud (cubre "Hi", "O", "Z")
    if [ ${#trimmed_name} -lt 3 ] || [ ${#trimmed_name} -gt 16 ]; then
        return 2
    fi
    
    # Caracteres no permitidos (cubre "myn@me")
...
# [MODIFICADO] para incluir los nuevos timers
cancel_player_timers() {
    local player_name="$1"
    
    # [MODIFICACIÓN] Añadidos los timers de nombre inválido
    local timer_types=("password_reminder" "password_kick" "ip_grace" "rank_application" "invalid_name_warning" "invalid_name_scheduler")
    
...
    done
    
    # [MODIFICACIÓN] Cancelar el bucle de kick (si está activo)
    if [ -n "${invalid_name_timers[$player_name]}" ]; then
        local pid="${invalid_name_timers[$player_name]}"
        if kill -0 "$pid" 2>/dev/null; then
            kill "$pid" 2>/dev/null
        fi
        unset invalid_name_timers["$player_name"]
    fi
    
...
# [NUEVA FUNCIÓN] (Caso 2: Advertencia a los 5s)
start_invalid_name_warning_timer() {
    local player_name="$1"
    local player_ip="$2"
    
    (
        sleep 5
        # Solo advertir si el jugador sigue conectado (con ese nombre)
        if [ -n "${connected_players[$player_name]}" ]; then
            log_debug "Sending invalid name warning to '$player_name'"
            execute_server_command "WARNING: $player_name, your name is invalid!"
            execute_server_command "Names must be 3-16 chars (A-Z, 0-9, _)."
            execute_server_command "If you don't change it, you will be kicked every 30s."
        fi
    ) &
    
    active_timers["invalid_name_warning_$player_name"]=$!
}

# [NUEVA FUNCIÓN] (Caso 2: Inicia el bucle de kick a los 30s)
start_invalid_name_kick_scheduler() {
    local player_name="$1"
    local player_ip="$2"
    
    (
        sleep 30 # Esperar 30 segundos desde la conexión
        
        if [ -n "${connected_players[$player_name]}" ]; then
            # Iniciar el bucle de kick AHORA
            start_invalid_name_kick_loop "$player_name" "$player_ip"
        fi
    ) &
    
    active_timers["invalid_name_scheduler_$player_name"]=$!
}

# [NUEVA FUNCIÓN] (Caso 2: El bucle de kick/ban temporal)
start_invalid_name_kick_loop() {
    local player_name="$1"
    local player_ip="$2"
    
    (
        # Bucle mientras el jugador esté conectado (con ese nombre)
        while [ -n "${connected_players[$player_name]}" ]; do
            log_debug "Invalid name kick loop: Kicking '$player_name' (IP: $player_ip)"
            
            # Kick
            execute_server_command "/kick $player_name"
            # Ban temporal 30s
...
            ) &
            
            # Esperar 30 segundos antes del siguiente kick
            sleep 30
        done
        log_debug "Invalid name kick loop STOPPED for '$player_name'"
        
    ) &
    
    # Guardar el PID del bucle (subshell) para poder matarlo
    invalid_name_timers["$player_name"]=$!
}

# [NUEVA FUNCIÓN] (Caso 1: Ban inmediato por nombre vacío)
handle_empty_player_name() {
    local player_ip="$1" player_hash="${2:-unknown}"
    
    print_error "EMPTY PLAYER NAME DETECTED: (IP: $player_ip, Hash: $player_hash)"
...
        
        # El /ban $player_ip ya debería kickearlo, pero enviamos /kick (vacío) por si acaso.
        send_server_command "$SCREEN_SESSION" "/kick " 
        
    else
        # Fallback si la IP es desconocida (baneo por nombre)
        print_warning "Banned empty player name: (fallback to name ban)"
        # [CORRECCIÓN] Llamada directa a 'send_server_command' para acción URGENTE
        send_server_command "$SCREEN_SESSION" "/ban \"\""
    fi
    
    return 1
}
...
            
            # [MODIFICACIÓN] Lógica de validación de nombre
            is_valid_player_name "$player_name"
            local name_status=$?
            
            if [ $name_status -eq 1 ]; then
                # Caso 1: Nombre vacío (Ban inmediato)
                handle_empty_player_name "$player_ip" "$player_hash"
                continue # No procesar más la conexión
                
            elif [ $name_status -eq 2 ]; then
                # Caso 2: Nombre inválido simple (Hi, O, Z, myn@me)
                log_debug "Invalid name detected: '$player_name' (IP: $player_ip). Starting timed warnings/kicks."
                
                # Marcar al jugador como conectado (para que los timers funcionen y se cancelen al salir)
                connected_players["$player_name"]=1
                player_ip_map["$player_name"]="$player_ip"
                # Marcar como 'pending' para que no obtenga rangos
                player_verification_status["$player_name"]="pending"
                
                # Iniciar los timers
                start_invalid_name_warning_timer "$player_name" "$player_ip"
                start_invalid_name_kick_scheduler "$player_name" "$player_ip"
                
                # Registrar al jugador si es nuevo, pero no hacer nada más
                local player_info=$(get_player_info "$player_name")
                if [ -z "$player_info" ]; then
                    log_debug "New player (invalid name): $player_name. Creating entry."
                    update_player_info "$player_name" "$player_ip" "NONE" "NONE" "NO" "NO"
                fi
                
                # Omitir el resto de la lógica de conexión (password, rangos, etc.)
                continue
            fi
            
            # Caso 0: Nombre Válido (continúa la lógica original)
            
...
            player_name=$(echo "$player_name" | xargs | tr '[:lower:]' '[:upper:]')
            
            # [MODIFICACIÓN] Chequeo de nombre válido antes de procesar comandos
            is_valid_player_name "$player_name"
            local name_status=$?
            
...
    done
    
    # [NUEVO] Limpiar timers de nombre inválido
    for player_name in "${!invalid_name_timers[@]}"; do
        local pid="${invalid_name_timers[$player_name]}"
        if kill -0 "$pid" 2>/dev/null; then
            kill "$pid" 2>/dev/null
        fi
