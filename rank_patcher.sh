...
COOLDOWN_DISCONNECT=15
COOLDOWN_LIST_CREATION=2
COOLDOWN_RANK_APPLICATION=5
COOLDOWN_SUPER_CLEANUP=10
...
# Estructuras de datos para gestión de estado
    ...
    if [ -n "${connected_players[$player_name]}" ] && [ "${player_verification_status[$player_name]}" = "verified" ]; then
        ...
        if [ -n "$player_info" ]; then
            ...
            if [ "$rank" != "NONE" ]; then
                ...
                # Paso 2: Esperar y aplicar el rango solo si está verificado
                (
                    sleep $COOLDOWN_RANK_APPLICATION
                    if [ -n "${connected_players[$player_name]}" ] && [ "${player_verification_status[$player_name]}" = "verified" ]; then
                        log_debug "Rank application timer completed, applying rank to verified player: $player_name"
                        apply_rank_to_connected_player "$player_name"
                    else
                        log_debug "Rank application timer completed but player $player_name not verified or disconnected"
                    fi
                ) &
...
start_disconnect_timer() {
    ...
    log_debug "Starting disconnect timer for: $player_name (Verification: ${player_verification_status[$player_name]})"
    
    # Si el jugador NO está verificado, cancelar inmediatamente y limpiar
    if [ "${player_verification_status[$player_name]}" != "verified" ]; then
        log_debug "Player $player_name not verified - immediately cleaning up lists"
        cleanup_empty_lists_after_disconnect "$player_name"
        return
    fi
    
    # Para jugadores verificados, usar el cooldown normal
    (
        local disconnect_time=$COOLDOWN_DISCONNECT
        log_debug "Waiting $disconnect_time seconds before rank removal for: $player_name"
        sleep $disconnect_time
        
        log_debug "Disconnect timer completed, removing rank for: $player_name"
...
start_super_disconnect_timer() {
    ...
    (
        sleep $COOLDOWN_SUPER_CLEANUP
        log_debug "SUPER timer completed, checking if other VERIFIED SUPER admins are connected"
...
# =============================================================================
# GESTIÓN DE TEMPORIZADORES Y VERIFICACIONES
# =============================================================================
...
cancel_player_timers() {
    ...
    for timer_type in "${timer_types[@]}"; do
        if [ -n "${active_timers["${timer_type}_$player_name"]}" ]; then
            local pid="${active_timers["${timer_type}_$player_name"]}"
            if kill -0 "$pid" 2>/dev/null; then
                kill "$pid" 2>/dev/null
                log_debug "Cancelled ${timer_type} timer for $player_name (PID: $pid)"
            fi
            unset active_timers["${timer_type}_$player_name"]
        fi
    done
...
handle_ip_change() {
    ...
    if [ -n "$player_info" ]; then
        ...
        log_debug "IP verification successful for $player_name - applying pending ranks"
        ...
        if [[ "$line" =~ Player\ Connected\ (.+)\ \|\ ([0-9a-fA-F.:]+)\ \|\ ([0-9a-f]+) ]]; then
            ...
            if [ -z "$player_info" ]; then
                ...
                if [ "$first_ip" = "UNKNOWN" ]; then
                    ...
                    log_debug "IP changed for $player_name: $first_ip -> $player_ip, requiring verification - NO RANK APPLICATION"
