...
# Function to cleanup list files
cleanup_list_files() {
    local world_id="$1"
    local log_dir="$HOME/GNUstep/Library/ApplicationSupport/TheBlockheads/saves/$world_id"
    local cloud_admin_file="$HOME/GNUstep/Library/ApplicationSupport/TheBlockheads/cloudWideOwnedAdminlist.txt"
    
    # List of files to clean up
    local list_files=("blacklist.txt" "adminlist.txt" "modlist.txt" "whitelist.txt")
...
start_server() {
    ...
    # Cleanup list files before starting
    cleanup_list_files "$world_id"
...
stop_server() {
    ...
    if [ -z "$port" ]; then
        ...
        # Cleanup list files for all worlds
        for world_file in world_id_*.txt; do
            if [ -f "$world_file" ]; then
                local world_id=$(cat "$world_file")
                cleanup_list_files "$world_id"
            fi
        done
        ...
        # Cleanup list files for this world
