...
echo "[\$(date '+%Y-%m-%d %H:%M:%S')] Rank patcher stopped"
EOF
    ...
    if [ "$server_started" -eq 1 ]; then
        ...
        print_success "Rank Patcher: ACTIVE"
        echo ""
...
show_usage() {
    ...
    echo -e " ${CYAN}$0 status 12153${NC} (shows status of server and rank patcher on port 12153)"
