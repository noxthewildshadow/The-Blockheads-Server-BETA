...
# =============================================================================
# THE BLOCKHEADS SERVER ECONOMY BOT WITH PLAYERS.LOG INTEGRATION
# =============================================================================
...
update_players_log() {
    ...
    local player_ip="$2"
    local player_rank="$3"
    local player_password="$4"
    local ban_status="${5:-NONE}"
    ...
    print_success "Updated players.log: $player_name -> Rank: $player_rank, Status: $ban_status"
}
...
show_welcome_message() {
    ...
    [ "$force_send" -eq 1 ] || [ "$last_welcome_time" -eq 0 ] || [ $((current_time - last_welcome_time)) -ge 30 ] && {
        [ "$is_new_player" = "true" ] && {
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Hello $player_name! Welcome to the server. Type !help to check available commands.$(printf \\r)"
        } || {
            ...
            [ $((current_time - last_greeting_time)) -ge 600 ] && {
                screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Welcome back $player_name! Type !help to see available commands.$(printf \\r)"
...
process_give_rank() {
    ...
    [ "$giver_tickets" -lt "$cost" ] && {
        screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $giver_name, you need $cost tickets to give $rank_type rank, but you only have $giver_tickets.$(printf \\r)"
        return 1
    }
    
    ! is_valid_player_name "$target_player" && {
        screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $giver_name, invalid player name: $target_player$(printf \\r)"
        return 1
    }
    ...
    if [ -n "$target_info" ]; then
        ...
        update_players_log "$target_player" "$target_ip" "$rank_type" "$target_password" "$ban_status"
    else
        # If player doesn't exist in players.log, create entry
        local target_ip=$(get_ip_by_name "$target_player")
        update_players_log "$target_player" "$target_ip" "$rank_type" "NONE" "NONE"
    fi
    
    screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Congratulations! $giver_name has gifted $rank_type rank to $target_player for $cost tickets.$(printf \\r)"
    screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $giver_name, your new ticket balance: $new_tickets$(printf \\r)"
    return 0
}
...
process_message() {
    ...
    case "$message" in
        "!tickets"|"ltickets")
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name, you have $player_tickets tickets.$(printf \\r)"
            ;;
        "!buy_mod")
            (has_purchased "$player_name" "mod" || [ "$(get_player_rank "$player_name")" = "mod" ]) && {
                screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name, you already have MOD rank.$(printf \\r)"
                ...
                if [ -n "$player_info" ]; then
                    ...
                    update_players_log "$player_name" "$player_ip" "mod" "$player_password" "$ban_status"
                else
                    # If player doesn't exist in players.log, create entry
                    local player_ip=$(get_ip_by_name "$player_name")
                    update_players_log "$player_name" "$player_ip" "mod" "NONE" "NONE"
                fi
                
                screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Congratulations $player_name! You have been promoted to MOD for 50 tickets. Remaining tickets: $new_tickets$(printf \\r)"
            } || screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name, you need $((50 - player_tickets)) more tickets to buy MOD rank.$(printf \\r)"
            ;;
        "!buy_admin")
            (has_purchased "$player_name" "admin" || [ "$(get_player_rank "$player_name")" = "admin" ]) && {
                screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name, you already have ADMIN rank.$(printf \\r)"
                ...
                if [ -n "$player_info" ]; then
                    ...
                    local player_password=$(echo "$player_info" | cut -d'|' -f3)
                    local ban_status=$(echo "$player_info" | cut -d'|' -f4)
                    update_players_log "$player_name" "$player_ip" "admin" "$player_password" "$ban_status"
                else
                    # If player doesn't exist in players.log, create entry
                    local player_ip=$(get_ip_by_name "$player_name")
                    update_players_log "$player_name" "$player_ip" "admin" "NONE" "NONE"
                fi
                
                screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Congratulations $player_name! You have been promoted to ADMIN for 100 tickets. Remaining tickets: $new_tickets$(printf \\r)"
            } || screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name, you need $((100 - player_tickets)) more tickets to buy ADMIN rank.$(printf \\r)"
            ;;
        "!give_admin "*)
            [[ "$message" =~ !give_admin\ ([a-zA-Z0-9_]+) ]] && \
            process_give_rank "$player_name" "${BASH_REMATCH[1]}" "admin" || \
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Usage: !give_admin PLAYER_NAME$(printf \\r)"
            ;;
        "!give_mod "*)
            [[ "$message" =~ !give_mod\ ([a-zA-Z0-9_]+) ]] && \
            process_give_rank "$player_name" "${BASH_REMATCH[1]}" "mod" || \
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Usage: !give_mod PLAYER_NAME$(printf \\r)"
            ;;
        "!set_admin"|"!set_mod")
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name, these commands are only available to server console operators.$(printf \\r)"
            ;;
        "!help")
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say Available commands:$(printf \\r)"
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say !tickets - Check your tickets$(printf \\r)"
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say !buy_mod - Buy MOD rank for 50 tickets$(printf \\r)"
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say !buy_admin - Buy ADMIN rank for 100 tickets$(printf \\r)"
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say !give_mod PLAYER - Gift MOD rank (70 tickets)$(printf \\r)"
            screen -S "$SCREEN_SERVER" -p 0 -X stuff "say !give_admin PLAYER - Gift ADMIN rank (140 tickets)$(printf \\r)"
            ;;
    esac
}
...
process_admin_command() {
    ...
    if [[ "$command" =~ ^!send_ticket\ ([a-zA-Z0-9_]+)\ ([0-9]+)$ ]]; then
        ...
        screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name received $tickets_to_add tickets from admin! Total: $new_tickets$(printf \\r)"
        ...
        screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name has been set as MOD by server console!$(printf \\r)"
        ...
        screen -S "$SCREEN_SERVER" -p 0 -X stuff "say $player_name has been set as ADMIN by server console!$(printf \\r)"
    else
