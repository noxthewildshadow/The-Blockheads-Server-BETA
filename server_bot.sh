#!/bin/bash
# =============================================================================
# THE BLOCKHEADS SERVER ECONOMY BOT WITH RANK UPDATES - OPTIMIZED
# =============================================================================
...
add_player_if_new() {
    local player_name="$1" player_ip="$2" economy_file="$3" players_log="$4" log_dir="$5"
    ...
    [ "$player_exists" = "false" ] && {
        ...
        if [ -z "$(get_player_info "$player_name" "$players_log")" ]; then
            update_player_info "$player_name" "$player_ip" "NONE" "NONE" "$players_log"
        fi
        
        give_first_time_bonus "$player_name" "$economy_file"
        return 0
    }
    return 1
}
...
give_first_time_bonus() {
    local player_name="$1" economy_file="$2"
    local current_time=$(date +%s) time_str="$(date '+%Y-%m-%d %H:%M:%S')"
    local current_data=$(read_json_file "$economy_file")
...
grant_login_ticket() {
    local player_name="$1" economy_file="$2" screen_server="$3"
    local current_time=$(date +%s) time_str="$(date '+%Y-%m-%d %H:%M:%S')"
    local current_data=$(read_json_file "$economy_file")
...
show_welcome_message() {
    local player_name="$1" is_new_player="$2" force_send="${3:-0}" screen_server="$4" economy_file="$5"
...
has_purchased() {
    local player_name="$1" item="$2" economy_file="$3"
    local current_data=$(read_json_file "$economy_file")
...
process_give_rank() {
    local giver_name="$1" target_player="$2" rank_type="$3" screen_server="$4" economy_file="$5" log_dir="$6" players_log="$7"
    local current_data=$(read_json_file "$economy_file")
    ...
    local target_ip=$(get_ip_by_name "$target_player" "$LOG_FILE")
    local player_info=$(get_player_info "$target_player" "$players_log")
    local password="NONE"
    ...
    update_player_info "$target_player" "$target_ip" "$rank_type" "$password" "$players_log"
...
process_message() {
    local player_name="$1" message="$2" player_ip="$3" screen_server="$4" economy_file="$5" log_dir="$6" players_log="$7"
    ! is_valid_player_name "$player_name" && return
    ...
    case "$message" in
        "!tickets"|"ltickets")
            ...
            ;;
        "!buy_mod")
            (has_purchased "$player_name" "mod" "$economy_file" || is_player_in_list "$player_name" "mod" "$log_dir") && {
                ...
                add_purchase "$player_name" "mod" "$economy_file"
                local time_str="$(date '+%Y-%m-%d %H:%M:%S')"
                current_data=$(echo "$current_data" | jq --arg player "$player_name" --arg time "$time_str" \
                ...
                update_player_info "$player_name" "$player_ip" "mod" "$password" "$players_log"
            ...
            ;;
        "!buy_admin")
            (has_purchased "$player_name" "admin" "$economy_file" || is_player_in_list "$player_name" "admin" "$log_dir") && {
                ...
                current_data=$(echo "$current_data" | jq --arg player "$player_name" --argjson tickets "$new_tickets" '.players[$player].tickets = $tickets')
                add_purchase "$player_name" "admin" "$economy_file"
                local time_str="$(date '+%Y-%m-%d %H:%M:%S')"
                current_data=$(echo "$current_data" | jq --arg player "$player_name" --arg time "$time_str" \
                ...
                update_player_info "$player_name" "$player_ip" "admin" "$password" "$players_log"
        ...
        "!give_admin "*)
            ...
            process_give_rank "$player_name" "${BASH_REMATCH[1]}" "admin" "$screen_server" "$economy_file" "$log_dir" "$players_log" || \
            send_server_command "$screen_server" "Usage: !give_admin PLAYER_NAME"
            ;;
        "!give_mod "*)
            [[ "$message" =~ !give_mod\ ([a-zA-Z0-9_]+) ]] && \
            process_give_rank "$player_name" "${BASH_REMATCH[1]}" "mod" "$screen_server" "$economy_file" "$log_dir" "$players_log" || \
...
process_admin_command() {
    local command="$1" economy_file="$2" screen_server="$3" players_log="$4" log_dir="$5"
    local current_data=$(read_json_file "$economy_file")
...
server_sent_welcome_recently() {
    local player_name="$1" log_file="$2" economy_file="$3"
...
monitor_log() {
    ...
    (
        while read -r admin_command < "$admin_pipe"; do
            ...
            process_admin_command "$admin_command" "$ECONOMY_FILE" "$SCREEN_SERVER" "$PLAYERS_LOG" "$LOG_DIR" || \
    ...
    tail -n 0 -F "$log_file" 2>/dev/null | filter_server_log | while read -r line; do
        if [[ "$line" =~ Player\ Connected\ (.+)\ \|\ ([0-9a-fA-F.:]+)\ \|\ ([0-9a-f]+) ]]; then
            ...
            add_player_if_new "$player_name" "$player_ip" "$ECONOMY_FILE" "$PLAYERS_LOG" "$LOG_DIR" && is_new_player="true"

            sleep 3

            ! server_sent_welcome_recently "$player_name" "$LOG_FILE" "$ECONOMY_FILE" && \
            show_welcome_message "$player_name" "$is_new_player" 1 "$SCREEN_SERVER" "$ECONOMY_FILE" || \
            print_warning "Server already welcomed $player_name"

            [ "$is_new_player" = "false" ] && grant_login_ticket "$player_name" "$ECONOMY_FILE" "$SCREEN_SERVER"
            continue
        fi
        ...
        if [[ "$line" =~ ([a-zA-Z0-9_]+):\ (.+)$ ]]; then
            ...
            local player_ip=$(get_ip_by_name "$player_name" "$LOG_FILE")
            add_player_if_new "$player_name" "$player_ip" "$ECONOMY_FILE" "$PLAYERS_LOG" "$LOG_DIR"
            process_message "$player_name" "$message" "$player_ip" "$SCREEN_SERVER" "$ECONOMY_FILE" "$LOG_DIR" "$PLAYERS_LOG"
            continue
        fi
