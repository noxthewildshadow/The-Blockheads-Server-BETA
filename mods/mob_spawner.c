/*
 * BLOCKHEADS ADMIN TOOLS: THE SUMMONER (Lite Edition)
 * Features:
 * - /spawn <mob> [qty] [baby]
 * - Pure Spawning Logic (No Taming/Memory Hacks).
 * - 100% Stable.
 * - FIX: Symbol Namespaced to prevent conflict.
 */
...
// --- Globals (Namespaced) ---
static CmdFunc Summoner_Real_HandleCmd = NULL;
static ChatFunc Summoner_Real_SendChat = NULL;

// --- MEMORY HELPERS (Namespaced) ---

id Summoner_GetObjectIvar(id obj, const char* ivarName) {
    if (!obj) return nil;
...
int Summoner_GetIntIvar(id obj, const char* ivarName) {
    if (!obj) return -1;
    Ivar ivar = class_getInstanceVariable(object_getClass(obj), ivarName);
    if (ivar) {
        ptrdiff_t offset = ivar_getOffset(ivar);
...
long long Summoner_GetLongIvar(id obj, const char* ivarName) {
    if (!obj) return 0;
    Ivar ivar = class_getInstanceVariable(object_getClass(obj), ivarName);
    if (ivar) {
        ptrdiff_t offset = ivar_getOffset(ivar);
...
const char* Summoner_GetStringText(id strObj) {
    if (!strObj) return "";
...
id Summoner_CreateNSString(const char* text) {
...
void Summoner_SendChat(id server, const char* msg) {
    if (server && Summoner_Real_SendChat) {
        id nsMsg = Summoner_CreateNSString(msg);
        Summoner_Real_SendChat(server, sel_registerName(SEL_CHAT), nsMsg, nil);
    }
}

int Summoner_GetNPCIDFromName(const char* name) {
    if (!name) return 0;
...
id Summoner_GetDynamicWorldFrom(id serverInstance) {
    if (!serverInstance) return nil;
    id worldObj = Summoner_GetObjectIvar(serverInstance, "world");
    if (!worldObj) return nil;
    return Summoner_GetObjectIvar(worldObj, "dynamicWorld");
}

// --- SEARCH ACTIVE PLAYER ---
id Summoner_GetActiveBlockhead(id dynWorld) {
    ...
    if (!playerList) playerList = Summoner_GetObjectIvar(dynWorld, SEL_NET_BHEADS);
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (obj) {
            long long pos = Summoner_GetLongIvar(obj, "pos");
            if (pos == 0) {
...
void Summoner_SummonNPC(id dynWorld, long long pos, int npcType, int qty, BOOL isAdult) {
...
// --- COMMAND HOOK (Namespaced) ---
id Summoner_Hook_HandleCommand(id self, SEL _cmd, id commandStr, id client) {
    const char* rawText = Summoner_GetStringText(commandStr);
      
    if (!rawText || strlen(rawText) == 0) {
        if (Summoner_Real_HandleCmd) return Summoner_Real_HandleCmd(self, _cmd, commandStr, client);
        return nil;
    }
    ...
    if (strncmp(text, "/spawn", 6) == 0) {
        id dynWorld = Summoner_GetDynamicWorldFrom(self);
        
        if (!dynWorld) {
            Summoner_SendChat(self, "Error: World not ready.");
            return nil;
        }
        ...
        if (!strType) {
            Summoner_SendChat(self, "Usage: /spawn <mob> [qty] [baby]");
            return nil;
        }

        int npcType = Summoner_GetNPCIDFromName(strType);
        if (npcType == 0) {
            Summoner_SendChat(self, "Error: Unknown mob type.");
            return nil;
        }
        ...
        if (strncasecmp(strType, "baby_", 5) == 0) {
            isAdult = 0;
            npcType = Summoner_GetNPCIDFromName(strType + 5); 
        }

        id targetBH = Summoner_GetActiveBlockhead(dynWorld);
        
        if (targetBH) {
            long long pos = Summoner_GetLongIvar(targetBH, "pos");
            // Pos fallback
            if (pos == 0) {
                 SEL selPos = sel_registerName(SEL_POS);
            ...
            if (pos != 0) {
                Summoner_SummonNPC(dynWorld, pos, npcType, qty, isAdult);
                ...
                snprintf(msg, sizeof(msg), "Summoned %d %s%s", 
                    ...
                    isAdult ? "Adult " : "Baby ", 
                    strType);
                Summoner_SendChat(self, msg);
            } else {
                Summoner_SendChat(self, "Error: Invalid player position.");
            }
        } else {
            Summoner_SendChat(self, "Player not found.");
        }
        return nil;
    }

    if (Summoner_Real_HandleCmd) {
        return Summoner_Real_HandleCmd(self, _cmd, commandStr, client);
    }
    return nil;
}

static void *Summoner_PatchThread(void *arg) {
    ...
    if (serverClass) {
        ...
        if (class_getInstanceMethod(serverClass, selCmd)) {
            Summoner_Real_HandleCmd = (CmdFunc)method_getImplementation(class_getInstanceMethod(serverClass, selCmd));
        ...
        if (class_getInstanceMethod(serverClass, selChat)) {
            Summoner_Real_SendChat = (ChatFunc)method_getImplementation(class_getInstanceMethod(serverClass, selChat));
        }
    }
    return NULL;
}
...
static void Summoner_Init() {
    pthread_t t;
    pthread_create(&t, NULL, Summoner_PatchThread, NULL);
}
