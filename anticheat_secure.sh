...
record_admin_offense() {
    local admin_name="$1" current_time
    current_time=$(date +%s)
    local offenses_data current_offenses last_offense_time
    offenses_data=$(read_json_file "$ADMIN_OFFENSES_FILE" 2>/dev/null || echo '{}')
...
send_server_command() {
    ...
    print_error "Could not send message to server: $1"
}
...
validate_authorization() {
    ...
    [ -f "$admin_list" ] && while IFS= read -r admin; do
        [[ -n "$admin" && ! "$admin" =~ ^[[:space:]]*# && ! "$admin" =~ "Usernames in this file" ]] || continue
        if ! grep -q -i "^$admin$" "$AUTHORIZED_ADMINS_FILE" 2>/dev/null; then
    ...
    [ -f "$mod_list" ] && while IFS= read -r mod; do
        [[ -n "$mod" && ! "$mod" =~ ^[[:space:]]*# && ! "$mod" =~ "Usernames in this file" ]] || continue
        if ! grep -q -i "^$mod$" "$AUTHORIZED_MODS_FILE" 2>/dev/null; then
