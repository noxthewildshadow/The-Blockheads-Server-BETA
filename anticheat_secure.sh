...
is_valid_player_name() {
    local player_name="$1"
    player_name="${player_name#"${player_name%%[![:space:]]*}"}"
    player_name="${player_name%"${player_name##*[![:space:]]}"}"
    [[ -z "$player_name" ]] && return 1
    if [ "${#player_name}" -lt 2 ] || [ "${#player_name}" -gt 16 ]; then
        return 1
    fi
...
handle_invalid_player_name() {
    ...
    local clean_name
    clean_name=$(echo "$player_name" | sed 's/\\/\\\\/g')
...
monitor_log() {
    ...
    tail -n 0 -F "$log_file" 2>/dev/null | filter_server_log | while read line; do
        ...
        if [[ "$line" =~ ([^:]+):\ \/(admin|mod)\ ([^[:space:]]+) ]]; then
            ...
            command_user="${command_user#"${command_user%%[![:space:]]*}"}"
            command_user="${command_user%"${command_user##*[![:space:]]}"}"
            target_player="${target_player#"${target_player%%[![:space:]]*}"}"
            target_player="${target_player%"${target_player##*[![:space:]]}"}"
