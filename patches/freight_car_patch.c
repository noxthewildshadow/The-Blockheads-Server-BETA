...
long long GetPosition(id obj) {
    ...
    return method ? ((PosReturnFunc)method)(obj, sel) : -1;
}
...
void SpawnDroppedItem(id dynWorld, long long pos) {
    ...
    if (method) ((SpawnFunc)method)(dynWorld, sel, pos, DROPPED_ITEM_ID, 0, 0, nil, nil, 1, 0, nil);
}
...
id Hook_Freight_InitLoad(id self, SEL _cmd, id world, id dynWorld, id saveDict, id chestDict, id cache) {
    ...
    if (loadedObj) {
        ...
        if (pos != -1) SpawnDroppedItem(dynWorld, pos);
        SoftRemoveObject(loadedObj);
    }
    return loadedObj;
}

id Hook_Freight_InitNet(id self, SEL _cmd, id world, id dynWorld, id cache, id netData) { return NULL; }
