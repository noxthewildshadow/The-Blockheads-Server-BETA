...
bool is_banned_portal(int id) { return (id >= 134 && id <= 139); }
...
id hook_PortalPlace(id self, SEL _cmd, id world, id dynWorld, long long pos, id cache, id item, unsigned char flipped, id saveDict, id client, id clientName) {
    if (item) {
        ...
        if (is_banned_portal(id)) return NULL; 
    }
    if (original_place) return original_place(self, _cmd, world, dynWorld, pos, cache, item, flipped, saveDict, client, clientName);
    return NULL;
}

id hook_PortalLoad(id self, SEL _cmd, id world, id dynWorld, id saveDict, id cache) {
    id loadedObj = NULL;
    if (original_load) loadedObj = original_load(self, _cmd, world, dynWorld, saveDict, cache);
    if (loadedObj) {
...
void hook_PortalUpdate(id self, SEL _cmd, float dt, bool isSim) {
    ...
    if (original_update) original_update(self, _cmd, dt, isSim);
}
...
__attribute__((constructor)) static void PBlocker_Entry() {
    pthread_t t; pthread_create(&t, NULL, PBlocker_InitThread, NULL);
}
