...
id hook_TP_Load(id self, SEL _cmd, id world, id dynWorld, id saveDict, id cache) {
    id obj = NULL;
    if (original_load) obj = original_load(self, _cmd, world, dynWorld, saveDict, cache);
    if (obj) {
...
__attribute__((constructor)) static void TBlocker_Entry() {
    pthread_t t; pthread_create(&t, NULL, TBlocker_InitThread, NULL);
}
